...
final class MemoryMonitor: @unchecked Sendable {
    ...
    struct MemoryInfo: Sendable {
        let totalPhysical: UInt64      // Total physical RAM (bytes)
        let rssBytes: UInt64           // Current process RSS (bytes)
        let freeEstimate: UInt64       // Estimated free memory (bytes)
        let activeSlots: Int           // Current slot count
    ...
    enum MemoryPressure: String, Sendable {
        case normal       // free > 4GB — can add slots
        case low          // 2GB < free < 4GB — hold steady
        case critical     // 1GB < free < 2GB — reduce slots
        case emergency    // free < 1GB — emergency release
    }

    /// Called on the monitoring thread when pressure changes or periodically.
    ...
    init(activeSlots: @escaping @Sendable () -> Int) {
        ...
        self._latestInfo = MemoryInfo(
            totalPhysical: ProcessInfo.processInfo.physicalMemory,
            rssBytes: Self.currentRSS(),
            freeEstimate: 0,
            activeSlots: 0
        )
        // Compute initial free estimate
    ...
    /// Get current process RSS using mach task_info.
    static func currentRSS() -> UInt64 {
        ...
        #endif
        // Fallback: estimate from ProcessInfo
